...
Read this first after a compaction or in a fresh session. It is the standing pickup for the `epk` lane; the numbered handoffs carry the day-by-day record (latest: highest `handoff-NNN-epk.md`). Updated 2026-09-20 (cast bios; the connector route). Before that: 2026-09-16 (after the 2.2 mirror).
...
- Worksheet: `press/epk.json`. Field 2.1 logline (Jordan's FilmFreeway text, rev 25); 2.2 synopsis; 7.1 cast bios (rev 34: Eric Pargac's and Jamal Dennis's exactly as Luke supplied them on 20 Sep, three paragraphs each, one paragraph per line in the field; Erik Orjiako's and Sandra McDaniels's still the 10 Sep drafts); 11.1 thanks (rev 32, 58 lines mirrored from the design).
...
3. Read the new box heights from the returned document (body boxes are 32 px per line plus 24.29), then `position_element` everything below. Page 7's rhythm: lockup to bio 58.37 px; bio box to next lockup one uniform gap (54.67 px on 20 Sep); ALSO BILLED 14.84 px further; rows at +45.33, +82.67, +120 with the names column 1.77 px higher; the last row's baseline on the 74 pt foot margin means its label box top at 2185.6 px.
...
- Cast bios: **Eric Pargac's and Jamal Dennis's are in, as Luke supplied them on 20 Sep** (worksheet rev 34, kit PDF `0958_20092026`, contract on Pages, Canva page 7 edited in place and committed; the wording matches each actor's self-written IMDb mini biography where the 10 Sep captures quote it). Erik Orjiako's and Sandra McDaniels's are still the 10 Sep drafts; the conservative proposal for those two awaits Luke's go. The two supplied bios do not end with the "In Killer of Men he plays" sentence the other two carry; Luke's call whether to match them. The EPK INFO Doc still shows the 10 Sep drafts for all four (it is also behind on 2.2, 3.1, 5.7 and 11.1).

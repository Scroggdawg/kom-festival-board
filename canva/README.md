...
| `tools/build-epk-canva.py` | the emitter: kit layout → ops JSON + flattened images, **measured in Libre Baskerville** (`--face libre`, the default; `--face baskerville` reproduces the 2026-09-11 first build's metrics) |
| `tools/check-canva-contract.py` | checks the contract before a build (lines overlapping on a baseline, a paragraph past the foot margin, the gap after each paragraph) and with `--render DIR` previews every page in Libre Baskerville |
| `press/assets/derived/fonts/libre-baskerville/` | the OFL Libre Baskerville files the emitter measures with; its README has the provenance |
...
Check: it prints `11 pages, N elements, 23 images, 19 paragraphs, 20 links attached, 0 unattached; ... measured in Libre Baskerville 2.005` (two of the twenty links are the `mailto:` rows, which the app places as plain text, so the Canva build carries 18 live links; the 23rd image is the director's headshot, 2026-09-11), and `press/assets/derived/canva/` holds 23 JPEGs. If links are unattached it exits non-zero; fix the layout, not the JSON. Then `venv/bin/python tools/check-canva-contract.py --render /tmp/contract` must report `0 problem(s)`; look at the pages it renders before spending Canva time on them.

**Why the face matters (2026-09-11):** the first full build was measured in macOS Baskerville while Canva draws Libre Baskerville, which is 9 to 28 per cent wider. Paragraphs reflowed longer (the director's statement clipped at the page foot, the cast bios ate their 48 pt gaps) and every handle placed after a lockup landed on it. The emitter now registers the Libre files under the kit's font names before the kit loads, so every fit, wrap, width and anchor is computed in the face Canva draws; the PDF and the .ai keep Baskerville. Two consequences: running text is emitted one element per paragraph (as one richtext, each blank line was a full empty line in Canva, 0.4 of a lead taller than the kit's gap), and the credit card steps its scale down until CREDITS is one page (0.96 in Libre; CREW and THANKS take the same scale, and CREW falls to its 16 pt cascade). Commit `canva/ops/epk-canva.json` and the derived images by explicit path and push, then wait about a minute and confirm `curl -sI https://scroggdawg.github.io/kom-festival-board/canva/ops/epk-canva.json` returns 200. The app cannot see uncommitted files.
...
In the app: Build all pages (or page by page). **Order:** each `addPage` lands after the previously added page (the insertion point follows the new page), even though `getCurrentPageMetadata()` reports the new page as not current, so Build all runs 1 to 11 (the default; the panel's order toggle exists because a reverse run was tried first on the strength of that reading and came out backwards). Build into a design that holds one blank page and delete the blank page afterwards. Any pilot page must be deleted (and its record forgotten) before the full build, or it lands out of sequence. **A page whose batch is rejected** (`addPage` takes at most 100 elements, an undocumented cap found on 2026-09-11: `bad_request: Please specify up to a maximum of 100 elements`; page 10, CREW, has 126) is left as an empty page carrying its background, because the per-element fallback cannot act on a page that is never reported current. Select that empty page in the editor, set Page n, and press **Place page N on the current page**: the elements go on with `addElementAtPoint`, one by one, onto the selected page. The app records each built page in the browser's localStorage and shows an "Export STATE" block; paste that block's values into `STATE.json` (`probes`, `pages_built`, `design.page_px`, `design.scale_px_per_pt`). `design.url` is copied by hand from the editor's address bar; the app cannot read it.

Check: eleven pages in order; per-page element counts equal `count` in the JSON; pages 3, 5, 8 and 9 screenshotted and compared to the PDF. The app's read back flags a `line` that wrapped and, since 2026-09-11, a `paragraph` that Canva set at least half a line longer than the `lines` the emitter measured.

**Rebuilding after the contract changes** (every text page changes when the measured face or the worksheet does): in the editor's grid view select page 1, shift-click the last page, Delete (or the page toolbar's trash, page by page); press Reset in the app (the progress record is keyed to the ops file's `generated`, so a new contract is not skipped anyway); Load the new ops file; pick the three font keys again; Build all into the one blank page that remains; Place page 10 on the selected empty page; delete the blank page; read back all pages; export.
...
2. If the JSON's `source.epk_rev` is older than `press/epk.json`'s `rev`, or its `measured_face` is not Libre Baskerville, decide with Luke whether to rebuild (Canva is the master after acceptance; a rebuild discards Canva-side edits on the pages it replaces).

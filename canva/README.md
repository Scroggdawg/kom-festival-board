...
In the app: Build all pages (or page by page). **Order:** `addPage` inserts the new page immediately after the page currently selected in the editor and the new page does not become current (the pilot logged `page_is_current false` on every build), so pages added in ascending order come out reversed. Build all therefore runs 11 down to 1 by default (the panel's order toggle), into a design that holds one blank page; delete the blank page afterwards. Any pilot page must be deleted (and its record forgotten) before the full build, or it lands out of sequence. The app records each built page in the browser's localStorage and shows an "Export STATE" block; paste that block's values into `STATE.json` (`probes`, `pages_built`, `design.page_px`, `design.scale_px_per_pt`). `design.url` is copied by hand from the editor's address bar; the app cannot read it.

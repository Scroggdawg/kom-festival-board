...
| Luke's review of the full build: the director's statement clips off the page foot, the cast bios run into the next lockup, and every Instagram handle and IMDB sits on its ROLE \| NAME lockup. One cause: the contract was measured in macOS Baskerville and Canva draws Libre Baskerville (Regular 28 per cent wider on running text, Bold 9 to 11 per cent), so paragraphs took more lines than the kit reserved and lines placed after a measured heading sat short of it | Emitter `--face libre` (default): the OFL Libre files (`press/assets/derived/fonts/libre-baskerville`, static instances of Google's variable font, widths identical to the v1.051 statics) are registered as `Bask` / `Bask-SB` / `Bask-B` before the kit loads, so the kit's own fit(), simpleSplit and stringWidth run in Libre; `measured_face` is written into the contract; `LINE_SLACK` drops to 1.10 in that mode. `tools/check-canva-contract.py` proves the contract (0 overlaps, nothing past the foot margin) and renders it in Libre before Canva is touched |
| A multi-paragraph richtext in Canva puts a full empty line at each `\n\n`, where the kit's gap is 0.6 of a lead: six of them on the statement page, 81 pt | Emitter writes one element per paragraph at the top the kit gave it, each carrying `lines` (the wrap it measured); `flagWrapped` now flags a paragraph read at least half a line taller than `lines * leading` |
| Measured in Libre, the CREDITS page (poster block plus cast rows) no longer fits one page at the card's 1.0 scale | `credit_pages()` steps the card's scale down (0.98, 0.96, ...) until `one_col_pages` accepts one page (0.96 in Libre); CREW and THANKS take the same scale so pages 9 to 11 stay one object; the PDF, measured in Baskerville, stays at 1.0 |
| The director's portrait in the first build was the value-graded set photograph Day3-43 (the stand-in from the taste pass); Luke sent the real studio portrait | It was on the Drive all along as `KOM Headshots-1.jpg` (same 14 Sep 2024 Hasselblad session as the four named headshots); now the fifth master in `press/assets/HEADSHOTS`, Drive twin `Director_Jordan_Betine.jpg`, `p05-e01.jpg` regenerated |
